...
![image-20220809151206877](images/image-20220809151206877.png)

* 堆（Heap）：此内存区域唯一目的就是存放对象实例，几乎所有的对象实例都在这里分配内存（包括数组）
* 栈（Stack），指虚拟机栈。用于存储局部变量等。局部变量表存放了编译期可知长度的各种基本数据类型(boolean、byte、char、short、int、float、long、double)、对象引用(reference类型，不等同与对象本身，是对象在堆内存的首地址)，方法执行完，自动释放
* 方法区（MethodArea)，用于存储已被虚拟机加载的类信息、常量、静态变量、即使编译器编译后的代码等数据
...
![image-20220809155659250](images/image-20220809155659250.png)
...
![image-20220809161102105](images/image-20220809161102105.png)
...
2. 具体使用：
   ...
   2. 当调用可变个数形参方法时，传入的参数的个数可以是 0个 1个 2个...
...
2. 值传递机制：
   ...
   如果参数是基本数据类型，此时参数赋值给形参的是实参真实存储的数据值
...
![image-20220809171208655](images/image-20220809171208655.png)
...
![image-20220809171753334](images/image-20220809171753334.png)
...
3. 递归一定要向**已知方向**递归，否则这种递归就变成了无穷递归，类似于死循环
...
**封装性的设计思想**：便于外界调用，从而提高系统的可扩展性、可维护性。**把该隐藏的隐藏起来，该暴露的暴露出来**
...
扩展：封装性的体现：1.如上  2.单例模式  3.不对外暴露的私有方法
...
Java权限修饰符 `public、protected、default(缺省)、private`置于类的成员定义之前，用来限定对象对该类成员的访问权限

![image-20220810093821877](images/image-20220810093821877.png)
...
3. 具体的，4 种权限都可以用来修饰类的内部结构：属性、方法、构造器、内部类、修饰类的话，只能使用：缺省、public
   ...
   总结封装性：java提供了4种权限修饰符来修饰类积累的内部结构，体现类及类的内部结构的可见性的方法
...
<img src="./images/image-20220810095022655.png" height=50% width=50% >
...
![image-20220810102040253](images/image-20220810102040253.png)
...
2. this修饰属性和方法，this理解为：当前对象，或当前正在创建的对象
   * 在类的方法中，我们可以使用 “this.属性” 或 “this.方法”的方式，调用当前对象的属性和方法，通常情况下，我们都选择省略 “this”。特殊情况下，如果方法的形参和类的属性同名，我们必须显式的使用 “this.变量” 的方式，表名此变量是属性，而非形参
   * 在类的构造器中，我们可以使用 “this.属性” 或 “this.方法”的方式，调用当前对象的属性和方法，通常情况下，我们都选择省略 “this”。特殊情况下，如果方法的形参和类的属性同名，我们必须显式的使用 “this.变量” 的方式，表名此变量是属性，而非形参
3. this调用构造器
   * 我们可以在类的构造器中，显式的使用 “this(形参列表)” 的方式，调用本类中重载的其他构造器
...
![image-20220810110041816](images/image-20220810110041816.png)
...
补充：在同一个包下，不能命名同名接口或同名类；不同包下，可以命名同名的接口、类



```java
//JDK中主要的包介绍
1.java.lang----包含一些 Java 语言的核心类，如 String、Math、Integer、System 和Thread，提供常用功能
...
![image-20220810155818791](images/image-20220810155818791.png)

![image-20220810155839332](images/image-20220810155839332.png)
